...
func (tc *TomoChainRpcClient) NonceAt(ctx context.Context, account tomochaincommon.Address, blockNumber string) (uint64, error) {
...
func (tc *TomoChainRpcClient) GetAccount(ctx context.Context, owner string, blockIdentifier *RosettaTypes.PartialBlockIdentifier) (res *RosettaTypes.AccountBalanceResponse, err error) {
	...
	err = tc.c.CallContext(ctx, &result, common.RPC_METHOD_GET_BALANCE, tomochaincommon.HexToAddress(owner), toBlockNumArg(big.NewInt(block.BlockIdentifier.Index)))
	if err != nil {
		return nil, err
	}
	...
	// attach nonce
	nonce, err := tc.NonceAt(ctx, tomochaincommon.HexToAddress(owner), toBlockNumArg(big.NewInt(block.BlockIdentifier.Index)))
	if err != nil {
		return nil, err
	}
