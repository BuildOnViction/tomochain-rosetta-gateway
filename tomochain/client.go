...
const (
	...
	tomoHTTPTimeout      = 120 * time.Second
)

type (
	// Client is an implementation of TomoChain client using local rpc/ipc connection.
	Client struct {
		sync.RWMutex
...
func (tc *Client) PendingNonceAt(ctx context.Context, account tomochaincommon.Address) (uint64, error) {
...
// This is the nonce that should be used for the next transaction.
func (tc *Client) NonceAt(ctx context.Context, account tomochaincommon.Address, blockNumber string) (uint64, error) {
	var result hexutil.Uint64
...
func (tc *Client) blockHeader(ctx context.Context, number *big.Int) (*tomochaintypes.Header, error) {
	var head *tomochaintypes.Header
...
func (tc *Client) GetBlockByHash(ctx context.Context, hash tomochaincommon.Hash) (*RosettaTypes.Block, error) {
	hashString := hash.Hex()
	return tc.Block(ctx, &RosettaTypes.PartialBlockIdentifier{
		Hash: &hashString,
	})
...
func (tc *Client) Balance(ctx context.Context, account *RosettaTypes.AccountIdentifier, blockIdentifier *RosettaTypes.PartialBlockIdentifier) (res *RosettaTypes.AccountBalanceResponse, err error) {
	...
	err = tc.c.CallContext(ctx, &result, common.RPC_METHOD_GET_BALANCE, tomochaincommon.HexToAddress(account.Address), toBlockNumArg(big.NewInt(block.BlockIdentifier.Index)))
	if err != nil {
		return nil, err
	}
	...
	// attach nonce
	nonce, err := tc.NonceAt(ctx, tomochaincommon.HexToAddress(account.Address), toBlockNumArg(big.NewInt(block.BlockIdentifier.Index)))
	if err != nil {
		return nil, err
	}
...
func (tc *Client) GetBlockTransactions(ctx context.Context, hash tomochaincommon.Hash) (res []*RosettaTypes.Transaction, err error) {
	hashString := hash.Hex()
...
func (tc *Client) GetBlockReward(ctx context.Context, hash tomochaincommon.Hash) (map[string]map[string]*big.Int, error) {
...
func (tc *Client) syncProgress(ctx context.Context) (*tomochain.SyncProgress, error) {
	var raw json.RawMessage
...
func (tc *Client) Call(
	...
	case common.RPC_METHOD_GET_TRANSACTION_RECEIPT:
		...
		receipt, err := tc.transactionReceipt(ctx, tomochaincommon.HexToHash(input.TxHash))
		if err != nil {
			return nil, err
		}
